// FileRRRutf8.pas : récupération des fichiers utf-8 autre que RW3
unit filerrrutf8;
...
function LitFichierCSVHorizontal(const NomFichier : String;NouveauFichier : boolean) : boolean;
var
    ligneList,listeLignes : TStringList;
    NbreCSV : integer;
...
begin
   try
   NbreCSV := 0;
   FinFichierCSV := FinFichierCSV or (ligneCourante>listeLignes.count);
   if finFichierCSV then exit;
   aLigne := listeLignes[ligneCourante];
   inc(ligneCourante);
...
begin // LitFichierCSVHorizontal
     ...
     listeLignes.LoadFromFile(NomFichier);
     ligneCourante := 0;
     premiereLigne := listeLignes[ligneCourante];
     inc(ligneCourante);
     NbreEnTete := 0;
     while (pos(';',premiereLigne)<=0) and
           (pos(',',premiereLigne)<=0) and
           (ligneCourante<listeLignes.count) do begin
             inc(NbreEnTete);
             premiereLigne :=listeLignes[ligneCourante];
             inc(ligneCourante);
     end;
     ...
     if NbreEnTete>0
        then begin
             commentaire := listeLignes[ligneCourante];
             inc(ligneCourante);
        end
        else commentaire := '';
     for i := 2 to NbreEnTete do
         inc(ligneCourante);
     if not ajoutePage then exit;
     repeat
           litUneLigne;
     until finFichierCSV or (ligneCourante>listeLignes.Count);
     LigneList.free;
     listeLignes.free;
     FileMode := fmOpenReadWrite;
...
begin
   ...
   ligneList.DelimitedText := listeLignes[ligneCourante];
   inc(ligneCourante);
   NbreCSV := ligneList.count;
   if NbreCSV>maxGrandeurs then nbreCSV := maxGrandeurs;
...
begin // GoodWill
     ...
     penteX := 1e-6;
     penteY := 0.001;
     while not finFichierCsv and not lecture do begin
           premiereLigne := listeLignes[ligneCourante];
           inc(ligneCourante);
...
begin  // Tektro
     ...
     if not ajoutePage then exit;
     while not(finfichierCSV) do extraitPoint;
     result := pages[pageCourante].nmes>128;
     penteY := penteY/probe;
...
var i : integer;
    uneLigne : string;
begin
    uneligne := listeLignes[ligneCourante];
    inc(ligneCourante);
    result := '';
...
begin  // secomam : csv avec tabulation !
     ...
     until (finFichierCSV) or (pos('Absorption',uneLigne)>0);
     if finFichierCSV then exit;
     if nouveauFichier then begin
...
begin // Chauvin Arnoux
    ...
    if nouveauFichier then begin
     ...
     premiereLigne := listeLignes[ligneCourante];// en fait la deuxième !
     inc(ligneCourante);
     separateurCSV := ';';
     litValeurs;
...
begin // LitFichierCSV
     ...
     if not isPasco then
        isPasco := compte(premiereLigne,'Exécuter')>5;  // Exécuter #1 #2 ...
     ...
     while (pos(';',premiereLigne)<=0) and
           ...
           not (finFichierCSV) do begin
           inc(NbreEnTete);
           premiereLigne := listeLignes[ligneCourante]; // deuxième... en fait
           inc(ligneCourante);
           if pos(';',premiereLigne)>0
              then separateurCSV := ';'
              else if pos(',',premiereLigne)>0
