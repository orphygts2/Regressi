...
{$endif USEAES}
  ...
  Classes, Contnrs, SysUtils, system.Types, ansiStrings,
...
begin
  ...
  if assigned(DTD) then begin
    j := 0;
    SetLength(DtdEntityNodes, j);
    for i := 0 to DTD.NodeCount - 1 do
      if DTD.Nodes[i] is TsdDtdEntity then begin
        inc(j);
        SetLength(DtdEntityNodes, j);
...
begin
  ...
  for i := 1 to length(S) do begin
    ...
    if LoCase(S[i]) <> LoCase(FUtf8Buffer[FUtf8CurrentIdx]) then begin
      Result := False;
      // revert
...
procedure TsdXmlParser.EncodeChunk;
  ...
  begin
    ...
    if FEncoding = seUTF16BE then begin
      for i := FRawFirstIdx div 2 to FRawLastIdx div 2 - 1 do begin
...
begin
  ...
  begin
    ...
    for i := 0 to FUtf8FirstIdx - 1 do begin
      // linefeed
...
begin
  ...
  for i := 0 to FUtf8CurrentIdx - 1 do begin
    // linefeed
    if FUtf8Buffer[i] = #$0A then
      inc(Result);
  end;
...
var
  i: integer;
begin
  for i := 0 to cCodePageInfoCount - 1 do begin
    if AnsiCompareText(ACharset, cCodePageInfo[i].Name) = 0 then begin
