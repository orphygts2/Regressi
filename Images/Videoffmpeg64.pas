...
uses
  ...
  Windows, SysUtils, Messages,  Classes, Vcl.Graphics, Controls,
...
uses regmain, regdde, graphvar, chronoU, ChronoBitmap, loupeU, rollingShutterU,
     avlib,
...
// Run a DOS program and retrieve its output dynamically while it is running.
  ...
  begin
      ...
      FProbe.Active := true;
      largeur := round(FProbe.VideoWidth);
      hauteur := round(FProbe.VideoHeight);
...
begin // ouvreFichier
     ...
     for i := 2 to pred(Grid.rowCount) do
         for j := 0 to pred(grid.colCount) do
             Grid.cells[j,i] := '';
     if LH_OK and (ImgPreview.Width>(largeurAff+32)) then
...
if MesureBtn.down
  ...
  if MesureAutoCB.checked
    then begin
       ...
       NChronoSE.value:= NImagesSE.value;
       lanceChrono;
    end
    else begin
      setBorne(bsMesure);
...
begin
    ...
    if not CharInSet(key,['0'..'9','.',',','E','e','+','-',
                     crSupprArr,crSuppr,crTab,crGauche,crDroite]) then key := #0
end;
...
begin
    ...
    //tempDirReg=Users/JMM/appData/Local/temp
end;
    ...
    if (FPreview.Active or FProbe.active) and (TrackBar.Position<TrackBar.SelEnd) then begin
     //    Screen.Cursor := crHourGlass;
         ...
         case BorneSelect of
            ...
            bsChrono,bsChronoInit : increment := NchronoSE.value;
            else increment := 1;
         end;
...
begin
    ...
    case BorneSelect of
         ...
         bsChrono : setCurseur(crHourGlass);
...
begin
   ...
   case methode of
        mPreview : begin
            ...
            imgPreview.Picture.Assign(FPreview.Bitmap);
            correctionRS.temps[index] := FPreview.TimeStamp/AV_TIME_BASE;// valeur exacte
            if not LH_OK then begin
               largeur := round(FPreview.bitmap.Width);
               hauteur := round(FPreview.bitmap.Height);
               LH_OK := true;
               EffectuerZoom(true);
            end;
        end;
        mProbe : begin
            imgPreview.Picture.Assign(FProbe.Bitmap);
            correctionRS.temps[index] := FProbe.TimeStamp/AV_TIME_BASE;// valeur exacte
        end;
   end;
   if index=0 then begin
      cc := GetCouleurFond(imgPreview.picture.bitmap);
      cc := CouleurComplementaire(cc);
...
begin
    ...
    bitmapLoad.pixelFormat := pf24bit;
...
begin
      ...
      if imageCourante=''
         then updatePreview(trackBar.Position)
             ...
             then begin
                updateImage(imageCourante);
                exit;
             end
             else imgPreview.Picture.Assign(bitmapC);
      case borneSelect of
           bsChrono : AffecteChrono;
           bsChronoInit : ;
           bsPlay : ;
...
begin
    ...
    if pasZoom>100 then pasZoom := 100;
    if pasZoom>1
    then begin
        ZoomBmp := pasZoom;
...
initialization
     FFMPEG_DLL_PATH := ExtractFilePath(application.exeName)+'Win64\';
finalization
     unloadLibs;
end.
