...
begin
  ...
  if not FileExists(NomFichier) then
     NomFichier := ExtractFilePath(application.exeName)+'regressi64.exe';
  if not FileExists(NomFichier) then begin
     showMessage('Placer ConfigRegressi dans le répertoire de Regressi');
     halt;
  end;
  nomFichier := ExtractFilePath(application.exeName)+'regressi.ini';
